...
def _dashboard_base_querysets(user, timeframe, event_slug):
    ...
    if event_slug:
        ...
        ledger = ledger.filter(transaction__payment_attempt__event__slug=event_slug)
...
def dashboard_home_context(user, *, timeframe='this_month', event_slug=''):
    ...
    def builder():
        events = list(competition_events_queryset(user).order_by('-start_at'))
...
def dashboard_analytics_context(user, *, timeframe='this_month', event_slug='', include_nominees=True):
    ...
    event_slug = _scoped_event_slug(user, event_slug)
    key = (
        f'dashboard-analytics:v3:user:{user.pk}:tf:{timeframe}:event:{event_slug or "all"}:'
...
def dashboard_revenue_summary_context(user):
    ...
    def builder():
        ...
        ledger_entries = LedgerEntry.objects.filter(transaction__payment_attempt__event__owner=user)
        ...
        return {
            'summary': {
                'confirmed_gross_revenue': totals['gross'],
...
def dashboard_revenue_lists_context(user, request_get):
    ...
    ledger_transactions = LedgerTransaction.objects.filter(payment_attempt__event__owner=user).select_related(
        'payment_attempt',
